...
(async () => {
    try {
        const base = 'content/library/articles'
        ...
        for(let file of files) {
            const path = file.split('.')[0].split('content')[1]
            const slug = file.split('/')[4].split('.')[0]
            const [item] = await fetch('http://localhost:3000/_content' + path).then(r => r.json())
            const { id: cover } = await directus.files.import({
                url: `https://yougotthis.io${path}/${item.cover}`,
